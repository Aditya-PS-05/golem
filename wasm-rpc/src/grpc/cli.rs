...
impl GrpcCliManager {
    ...
    pub fn generate_wit_for_dependencies(&mut self, config_path: &Path) -> GrpcResult<Vec<String>> {
        ...
        if let Some(dependencies) = &config.dependencies {
            let base_dir = config_path.parent().ok_or_else(|| {
                GrpcError::InvalidConfiguration("Invalid config path".to_string())
            })?;
            ...
            for (dep_name, dep_config) in dependencies {
                if dep_config.dependency_type == "grpc" {
                    ...
                    std::fs::create_dir_all(&wit_output_dir).map_err(|e| {
                        GrpcError::IoError(format!("Failed to create WIT output directory: {}", e))
                    })?;

                    // Read and convert proto file
                    let proto_content = std::fs::read_to_string(&proto_path).map_err(|e| {
                        GrpcError::IoError(format!(
                    ...
                    // Write WIT file
                    std::fs::write(&wit_file_path, wit_content).map_err(|e| {
                        GrpcError::IoError(format!("Failed to write WIT file: {}", e))
                    })?;
    ...
    pub fn list_grpc_dependencies(
        &self,
        config_path: &Path,
    ) -> GrpcResult<Vec<(String, GrpcDependencyConfig)>> {
        let config = Self::load_golem_config(config_path)?;
            ...
            for (dep_name, dep_config) in dependencies {
                if dep_config.dependency_type == "grpc" {
                    ...
                    if !proto_path.exists() {
                        errors.push(format!(
                            "Proto file not found for dependency '{}': {}",
                            dep_name,
                            proto_path.display()
                        ));
                        continue;
                    ...
                    if let Ok(proto_content) = std::fs::read_to_string(&proto_path) {
                        ...
                        if let Err(e) = parser.parse(&proto_content) {
                            errors.push(format!(
                                "Invalid proto file for dependency '{}': {}",
                                dep_name, e
                            ));
                        }
                    } else {
                        errors.push(format!(
                            "Cannot read proto file for dependency '{}': {}",
                            dep_name,
                            proto_path.display()
                        ));
                    }
                    ...
                    } else if !dep_config.endpoint.starts_with("http://")
                        && !dep_config.endpoint.starts_with("https://")
                    {
                        errors.push(format!(
                            "Invalid endpoint URL format for dependency '{}': {}",
                            dep_name, dep_config.endpoint
                        ));
                    }
...
#[cfg(test)]
mod tests {
    use super::{GolemYamlConfig, GrpcAuthConfig, GrpcCliManager, GrpcDependencyConfig};
    use std::collections::HashMap;
    ...
    fn test_cli_manager_operations() {
        ...
        manager
            .add_grpc_dependency(config_path, "test-service".to_string(), dep_config)
            .unwrap();
        ...
        // Remove dependency
        let removed = manager
            .remove_grpc_dependency(config_path, "test-service")
            .unwrap();
        assert!(removed);
