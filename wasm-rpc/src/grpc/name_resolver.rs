...
impl NameResolver {
            ...
            if ch.is_uppercase() && !first {
                ...
                if let Some(&next_ch) = chars.peek() {
                    if next_ch.is_lowercase()
                        || result.chars().last().is_some_and(|c| c.is_lowercase())
                    {
                        result.push('-');
                    }
                }
    ...
    pub fn register_name_mapping(
        ...
        self.name_mappings
            .insert(proto_name, final_wit_name.clone());
...
mod tests {
    ...
    fn test_reserved_words() {
        ...
        assert_eq!(
            resolver.proto_to_wit_field_name("normal_field"),
            "normal-field"
        );
    }
    ...
    fn test_name_conflict_resolution() {
        ...
        let name1 = resolver
            .register_name_mapping("proto1".to_string(), "test".to_string())
            .unwrap();
        assert_eq!(name1, "test");

        // Register a conflicting name
        let name2 = resolver
            .register_name_mapping("proto2".to_string(), "test".to_string())
            .unwrap();
        assert_eq!(name2, "test-2");

        // Register another conflicting name
        let name3 = resolver
            .register_name_mapping("proto3".to_string(), "test".to_string())
            .unwrap();
